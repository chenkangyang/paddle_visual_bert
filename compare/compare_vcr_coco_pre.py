...
torch_model_name = "uclanlp/visualbert-vcr-coco-pre"
paddle_model_name = "visualbert-vcr-coco-pre"
...
with torch.no_grad():
    torch_outputs = torch_model(**torch_inputs, labels=torch_labels, sentence_image_labels=torch_sentence_image_labels)

torch_loss = torch_outputs.loss.cpu().detach().numpy()
torch_prediction_logits = torch_outputs.prediction_logits.cpu().detach().numpy()
torch_seq_relationship_logits = torch_outputs.seq_relationship_logits.cpu().detach().numpy()
...
print("torch_seq_relationship_logits shape:{}".format(torch_seq_relationship_logits.shape))
...
paddle_inputs = paddle_tokenizer(text_masked, max_seq_len=128, pad_to_max_seq_len=True, return_attention_mask=True)
...
max_length  = paddle_inputs["input_ids"].shape[-1] + paddle_visual_embeds.shape[-2]
paddle_labels = paddle.to_tensor(paddle_tokenizer(text, max_seq_len=max_length, pad_to_max_seq_len=True)["input_ids"])
paddle_sentence_image_labels = paddle.to_tensor(1).unsqueeze(0) # Batch_size

with paddle.no_grad():
    paddle_outputs = paddle_model(**paddle_inputs, labels = paddle_labels, sentence_image_labels = paddle_sentence_image_labels)
...
if not return_dict:
    ...
    paddle_prediction_logits = paddle_outputs['prediction_logits'].cpu().detach().numpy()
    paddle_seq_relationship_logits = paddle_outputs['seq_relationship_logits'].cpu().detach().numpy()

print("paddle_loss:{}".format(paddle_loss))
print("paddle_prediction_logits shape:{}".format(paddle_prediction_logits.shape))
print("paddle_prediction_logits:{}".format(paddle_prediction_logits))
print("paddle_seq_relationship_logits shape:{}".format(paddle_seq_relationship_logits.shape))
print("paddle_seq_relationship_logits:{}".format(paddle_seq_relationship_logits))

# ==============================================================================
assert torch_prediction_logits.shape == paddle_prediction_logits.shape, "the output logits should have the same shape, but got : {} and {} instead".format(paddle_prediction_logits.shape, paddle_prediction_logits.shape)
assert torch_seq_relationship_logits.shape == paddle_seq_relationship_logits.shape, "the output logits should have the same shape, but got : {} and {} instead".format(torch_seq_relationship_logits.shape, paddle_seq_relationship_logits.shape)
