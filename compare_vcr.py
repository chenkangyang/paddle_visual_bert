...
# torch output
import torch
from transformers import BertTokenizer as PTBertTokenizer, VisualBertForMultipleChoice as PTVisualBertForMultipleChoice

import paddle
from paddlenlp.transformers import BertTokenizer as PDBertTokenizer, VisualBertForMultipleChoice as PDVisualBertForMultipleChoice
import numpy as np

torch_model = PTVisualBertForMultipleChoice.from_pretrained(torch_model_name)
torch_tokenizer = PTBertTokenizer.from_pretrained("bert-base-uncased")
torch_model.eval()

torch_inputs = torch_tokenizer([[text, text], [choice0, choice1]], return_tensors="pt", padding=True, max_length=128)
torch_inputs = {k:v.unsqueeze(0) for k,v in torch_inputs.items()}

torch_visual_embeds = torch.ones([100,512]).unsqueeze(0)
torch_visual_embeds = torch_visual_embeds.expand(1, 2, *torch_visual_embeds.shape)
...
torch_labels = torch.tensor(0).unsqueeze(0) # choice0 is correct (according to Wikipedia ;)), batch size 1
with torch.no_grad():
...
# ========================================================================================================
paddle_model = PDVisualBertForMultipleChoice.from_pretrained(paddle_model_name, num_classes=1)
paddle_tokenizer = PDBertTokenizer.from_pretrained("bert-base-uncased")
paddle_model.eval()

paddle_inputs = paddle_tokenizer.batch_encode(batch_text_or_text_pairs=[[text, text], [choice0, choice1]], max_seq_len=128, pad_to_max_seq_len=True, return_attention_mask=True)
...
for item in paddle_inputs: 
    paddle_input_ids_list.append(paddle.to_tensor(item['input_ids']).unsqueeze(0))
    paddle_token_type_ids_list.append(paddle.to_tensor(item['token_type_ids']).unsqueeze(0))
    paddle_attention_mask_list.append(paddle.to_tensor(item['attention_mask']).unsqueeze(0))

paddle_inputs_dict = {
    "input_ids": paddle.concat(paddle_input_ids_list, axis=0).unsqueeze(0),
    "token_type_ids": paddle.concat(paddle_token_type_ids_list, axis=0).unsqueeze(0),
    "attention_mask": paddle.concat(paddle_attention_mask_list, axis=0).unsqueeze(0)
} 

paddle_visual_embeds = paddle.ones([100,512]).unsqueeze(0)
paddle_visual_embeds = paddle_visual_embeds.expand(shape=[1, 2, *paddle_visual_embeds.shape])
...
paddle_labels = paddle.to_tensor(0).unsqueeze(0) # choice0 is correct (according to Wikipedia ;)), batch size 1
